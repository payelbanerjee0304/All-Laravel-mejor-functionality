                        ...
                        @foreach ($subtask as $questionIndex => $questionData)
                            @if (is_array($questionData))
                                <div class="question-section">
                                    ...
                                    <div class="form-group dropdown-input-container" style="display: {{ $questionData['selecttype'] == 'dropdown' ? 'block' : 'none' }};">
                                        ...
                                        @foreach ($questionData['options'] as $optionIndex => $option)
                                            @foreach ($option['parttask'] as $parttaskIndex => $parttask)
                                                <div id="parttask-{{ $option['text'] }}-{{ $subtaskIndex }}-{{ $questionIndex }}" class="parttask" data-parttask-question="{{ $parttask['question'] }}">
                                                    ...
                                                    @endif
                                                    <div class="form-group" style="display: {{ $parttask['parttask_togglefile'] == 1 ? 'block' : 'none' }};">
                                                        <label for="">Upload Parttask Toggle File</label>
                                                        <input type="file" name="parttask[{{ $subtaskIndex }}][{{ $questionIndex }}][parttask_togglefile]" id="" class="form-control">
                                                    </div>
                                                </div>
                                            @endforeach
                                        @endforeach
                                    ...
                                    {{-- <input type="text" name="" id="" value="{{ $questionData['togglefile']}}"> --}}
                                    <div class="form-group" style="display: {{ $questionData['togglefile'] == 1 ? 'block' : 'none' }};">
                                        <label for="questions[{{ $subtaskIndex }}][{{ $questionIndex }}][togglefile]">Upload Toggle File</label>
                                        <input type="file" name="questions[{{ $subtaskIndex }}][{{ $questionIndex }}][togglefile]" id="togglefile-{{ $subtaskIndex }}-{{ $questionIndex }}" class="form-control">
                                    </div>
                                </div>
                            @endif
                        @endforeach
                ...
                if ($(this).attr('type') === 'file') {
                    ...
                    if ($(this).hasClass('select-type')) {
                        ...
                        if (selectedValue) {
                            ...
                            var parttaskQuestion = parttaskSection.data('parttask-question');
                            ...
                            parttaskSection.find('input[type="text"], input[type="radio"]:checked, input[type="checkbox"]:checked').each(function() {
                                ...
                                if (parttaskAnswer) { 
                                    parttaskData.push(parttaskAnswer);
                                }
                            });


                            if (parttaskData.length > 0) {
                                formData.append('questions[' + subtaskIndex + '][' + $(this).data('question-index') + '][Answer][0][answertext]', answerText);
                                formData.append('questions[' + subtaskIndex + '][' + $(this).data('question-index') + '][Answer][0][parttask][parttask_question]', parttaskQuestion);
                                ...
                                var parttaskToggleFileInput = parttaskSection.find('input[type="file"]');
                                if (parttaskToggleFileInput.length > 0 && parttaskToggleFileInput[0].files.length > 0) {
                                    var toggleFile = parttaskToggleFileInput[0].files[0];
                                    formData.append('questions[' + subtaskIndex + '][' + $(this).data('question-index') + '][Answer][0][parttask][parttask_togglefile]', toggleFile);
                                }
                            }
                        }
                    } else {
